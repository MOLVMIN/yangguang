  ...
  <div>
      ...
      <TabPane label="我的申请列表" name="name1">
        <Card>
          <div class="search-con">
            ...
            <Button v-for="item in permissionBtns" v-if="item.p_id === 150" @click="handleAdd" class="add-btn" type="primary">{{ item.p_name }}</Button>
          </div>
          <Table
            ref="tablesAsk"
            :data="tableData"
    ...
    <Modal width="800" v-model="showModal" :mask-closable="false">
      ...
      <Row>
        <Col span="12" class="form-container-item">
          <label>起止时间：</label>
          <DatePicker v-model="aoDate" type="datetimerange" format="yyyy-MM-dd HH:mm" placeholder="请选择外勤起止时间" style="width: 100%"></DatePicker>
        </Col>
      </Row>
      <Row>
        <Col span="24" class="form-container-item">
          <label style="width: 125px">外勤原因：</label>
          <Input v-model="aoResult" type="textarea" placeholder="请填写外勤原因" />
        </Col>
      </Row>
      <div slot="footer">
    ...
    <Modal width="400" v-model="showSubmitModal" :mask-closable="false">
      ...
      <Row class="form-container">
        <Col span="24" class="form-container-item">
        <label>主任审批：</label>
        <Select :disabled="isView" v-model="directorId" placeholder="请选择审批人">
          <Option v-for="item in directorList" :value="item.id" :key="`search-col-${item.id}`">{{ item.username }}</Option>
        </Select>
        </Col>
        <Col span="24" class="form-container-item">
        <label>干事审批：</label>
        <Select :disabled="isView" v-model="generalId" placeholder="请选择审批人">
  ...
  export default {
    data () {
      return {
        ...
        aoResult: '',
        columns: [
          {title: '序号', align: 'center', key: 'ao_id', sortable: true},
          {title: '申请人', align: 'center', key: 'username'},
          {title: '外出原因', align: 'center', key: 'ao_result'},
          {title: '开始日期', align: 'center', key: 'ao_startdate'},
          {title: '结束日期', align: 'center', key: 'ao_enddate'},
          {
            title: '操作',
            key: 'handle',
            align: 'center',
              ...
              for (var i = 0; i < this.handleBtns.length; i++) {
                ...
                if (this.handleBtns[i].p_id === 151) {
                  btns.push(h('Button', {
                    ...
                    on: {
                      click: () => {
                        this.showOneEntity(params.row.ao_id)
                        this.showModal = true
                        this.modalTitle = '申请修改'
                        this.isView = false
                        this.isEdit = true
                        this.editId = params.row.ao_id
                      }
                    }
                  }, '修改'))
                } else if (this.handleBtns[i].p_id === 152) {
                  btns.push(h('Button', {
                    props: {
                      type: 'info',
                      size: 'small'
                ...
                } else if (this.handleBtns[i].p_id === 153) {
                  btns.push(h('Button', {
                    props: {
                      type: 'error',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.deleteEntity(params.row.ao_id)
                      }
                    }
                  }, '删除'))
                } else if (this.handleBtns[i].p_id === 154) {
                  btns.push(h('Button', {
                    props: {
                      type: 'warning',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.restoreEntity(params.row.ao_id)
                      }
                    }
                  }, '恢复'))
                } else if (this.handleBtns[i].p_id === 155) {
                  btns.push(h('Button', {
                    props: {
                      type: 'success',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.submitEvent(params.row.ao_id)
                      }
                    }
                  }, '提交'))
                }
    ...
    methods: {
      ...
      addEntity: function () {
        ...
        axios.request({
          ...
          data: {
            appUser: getUserId(),
            aoResult: _this.aoResult,
            aoStartdate: dateToStr(new Date(_this.aoDate[0]), 'yyyy-MM-dd hh:MM:ss'),
            aoEnddate: dateToStr(new Date(_this.aoDate[1]), 'yyyy-MM-dd hh:MM:ss'),
            aoDates: dateDiff(dateToStr(new Date(_this.aoDate[0]), 'yyyy-MM-dd hh:MM:ss'), dateToStr(new Date(_this.aoDate[1]), 'yyyy-MM-dd hh:MM:ss'))
          },
          method: 'post'
        }).then(res => {
      ...
      showOneEntity: function (entityId) {
        ...
        axios.request({
          ...
          _this.aoResult = res.data.content.ao_result,
          _this.aoDate = [new Date(res.data.content.ao_startdate), new Date(res.data.content.ao_enddate)]
        })
      },
      ...
      getPermissionBtns: function () {
        ...
        for (var item of common.permission) {
          if (item.p_fid === this.$route.meta.id) {
            ...
            if (item.p_id === 151 || item.p_id === 152 || item.p_id === 153 || item.p_id === 154 || item.p_id === 155) {
