      ...
      <TabPane label="我的申请列表" name="name1">
        <Card>
          <div class="search-con">
            ...
            <Button v-for="item in permissionBtns" v-if="item.p_id === 134" @click="handleAdd" class="add-btn" type="primary">{{ item.p_name }}</Button>
          </div>
          <Table
            ref="tablesAsk"
            :data="tableData"
    ...
    <Modal width="800" v-model="showModal" :mask-closable="false">
      ...
      <Row class="form-container">
        <Col span="12" class="form-container-item">
          <label>申购部门：</label>
          <Select v-model="agDepart" placeholder="请填写原部门">
      ...
      <Row class="form-container">
        ...
        <label style="width: 125px">申购原因：</label>
        <Input v-model="agResult" type="textarea" placeholder="请选择转正时间" />
        </Col>
      </Row>
      <div slot="footer">
  ...
  export default {
    data () {
      return {
        ...
        agGoods: '',
        columns: [
          {title: '序号', align: 'center', key: 'ag_id', sortable: true},
              ...
              for (var i = 0; i < this.handleBtns.length; i++) {
                ...
                if (this.handleBtns[i].p_id === 135) {
                  btns.push(h('Button', {
                    ...
                    on: {
                      click: () => {
                        this.showOneEntity(params.row.ag_id)
                        this.showModal = true
                        this.modalTitle = '申请修改'
                        this.isView = false
                        this.isEdit = true
                        this.editId = params.row.ag_id
                      }
                    }
                  }, '修改'))
                } else if (this.handleBtns[i].p_id === 136) {
                  btns.push(h('Button', {
                    props: {
                      type: 'info',
                      size: 'small'
                ...
                } else if (this.handleBtns[i].p_id === 137) {
                  btns.push(h('Button', {
                    props: {
                      type: 'error',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.deleteEntity(params.row.ag_id)
                      }
                    }
                  }, '删除'))
                } else if (this.handleBtns[i].p_id === 138) {
                  btns.push(h('Button', {
                    props: {
                      type: 'warning',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.restoreEntity(params.row.ag_id)
                      }
                    }
                  }, '恢复'))
                } else if (this.handleBtns[i].p_id === 139) {
                  btns.push(h('Button', {
                    props: {
                      type: 'success',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.submitEvent(params.row.ag_id)
                      }
                    }
                  }, '提交'))
                }
      ...
      getPermissionBtns: function () {
        ...
        for (var item of common.permission) {
          if (item.p_fid === this.$route.meta.id) {
            ...
            if (item.p_id === 135 || item.p_id === 136 || item.p_id === 137 || item.p_id === 138 || item.p_id === 139) {
