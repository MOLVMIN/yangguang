      ...
      <TabPane label="我的申请列表" name="name1">
        <Card>
          <div class="search-con">
            ...
            <Button v-for="item in permissionBtns" v-if="item.p_id === 73" @click="handleAdd" class="add-btn" type="primary">{{ item.p_name }}</Button>
          </div>
          <Table
            ref="tablesAsk"
            :data="tableData"
    ...
    <Modal width="800" v-model="showModal" :mask-closable="false">
      ...
      <Row>
        <Col span="12" class="form-container-item">
          <label>离职类型：</label>
          <Select :disabled="isView" v-model="lbType" placeholder="请选择离职类型">
            <Option v-for="item in lbTypeArr" :value="item.key" :key="`search-col-${item.key}`">{{ item.title }}</Option>
          </Select>
        </Col>
        <Col span="12" class="form-container-item">
          <label>申请离职日期：</label>
          <DatePicker :readonly="isView" v-model="lbAppDate" type="date" format="yyyy-MM-dd" placeholder="请选择申请离职时间" style="width: 100%"></DatePicker>
        </Col>
        <Col span="12" class="form-container-item">
          <label>核准离职日期：</label>
          <DatePicker :readonly="isView" v-model="lbLeaveDate" type="date" format="yyyy-MM-dd" placeholder="请选择核准离职时间" style="width: 100%"></DatePicker>
        </Col>
      </Row>
      <Row>
        <Col span="24" class="form-container-item">
        <label style="width: 125px">离职原因：</label>
        <Input :readonly="isView" v-model="lbReason" type="textarea" style="width: 100%;" />
        </Col>
      </Row>
      <div slot="footer">
  ...
  export default {
    data () {
      return {
        ...
        lbReason: '',
        columns: [
          {title: '序号', align: 'center', key: 'lb_id', sortable: true},
          {title: '离职人', align: 'center', key: 'username'},
          {title: '申请离职日期', align: 'center', key: 'lb_app_date'},
          {title: '核准离职日期', align: 'center', key: 'lb_leave_date'},
          {
            title: '操作',
            key: 'handle',
            align: 'center',
            ...
            render: (h, params) => {
              ...
              for (var i = 0; i < this.handleBtns.length; i++) {
                ...
                if (this.handleBtns[i].p_id === 74) {
                  btns.push(h('Button', {
                    props: {
                      type: 'primary',
                      size: 'small',
                      disabled: params.row.data_level === 5 || params.row.process_id
                    },
                    style: {
                      margin: '2px'
                    },
                    on: {
                      click: () => {
                        this.showOneEntity(params.row.lb_id)
                        this.showModal = true
                        this.modalTitle = '申请修改'
                        this.isView = false
                        this.isEdit = true
                        this.editId = params.row.lb_id
                      }
                    }
                  }, '修改'))
                } else if (this.handleBtns[i].p_id === 75) {
                  btns.push(h('Button', {
                    props: {
                      type: 'info',
                      size: 'small'
                ...
                } else if (this.handleBtns[i].p_id === 76) {
                  btns.push(h('Button', {
                    props: {
                      type: 'error',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.deleteEntity(params.row.lb_id)
                      }
                    }
                  }, '删除'))
                } else if (this.handleBtns[i].p_id === 77) {
                  btns.push(h('Button', {
                    props: {
                      type: 'warning',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.restoreEntity(params.row.lb_id)
                      }
                    }
                  }, '恢复'))
                } else if (this.handleBtns[i].p_id === 78) {
                  btns.push(h('Button', {
                    props: {
                      type: 'success',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.submitEvent(params.row.lb_id)
                      }
                    }
                  }, '提交'))
                }
      ...
      addEntity: function () {
        ...
        axios.request({
          ...
          data: {
            appUser: getUserId(),
            lbType: _this.lbType,
            lbAppDate: dateToStr(new Date(_this.lbAppDate), 'yyyy-MM-dd hh:MM:ss'),
            lbLeaveDate: dateToStr(new Date(_this.lbLeaveDate), 'yyyy-MM-dd hh:MM:ss'),
            lbReason: _this.lbReason
          },
          method: 'post'
        }).then(res => {
      ...
      showOneEntity: function (entityId) {
        ...
        axios.request({
          ...
          method: 'post'
            ...
            _this.lbType = res.data.content.lb_type,
            _this.lbAppDate = dateToStr(new Date(res.data.content.lb_app_date), 'yyyy-MM-dd hh:MM:ss'),
            _this.lbLeaveDate = dateToStr(new Date(res.data.content.lb_leave_date), 'yyyy-MM-dd hh:MM:ss'),
            _this.lbReason = res.data.content.lb_reason
        })
      },
      ...
      getPermissionBtns: function () {
        ...
        for (var item of common.permission) {
          if (item.p_fid === this.$route.meta.id) {
            ...
            if (item.p_id === 74 || item.p_id === 75 || item.p_id === 76 || item.p_id === 77 || item.p_id === 78) {
