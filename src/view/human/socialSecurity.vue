      ...
      <TabPane label="我的申请列表" name="name1">
        <Card>
          <div class="search-con">
            ...
            <Button v-for="item in permissionBtns" v-if="item.p_id === 174" @click="handleAdd" class="add-btn" type="primary">{{ item.p_name }}</Button>
          </div>
          <Table
            ref="tablesAsk"
            :data="tableData"
    ...
    <Modal width="800" v-model="showModal" :mask-closable="false">
      ...
      <Row class="form-container">
        <Col span="12" class="form-container-item">
          <label>是否有社保卡：</label>
          <Select :disabled="isView" v-model="asIsaccount" placeholder="请选择" clearable @on-clear="searchKeyChange">
            <Option v-for="item in [{ key: true, title: '是' }, { key: false, title: '否' }]" :value="item.key" :key="`search-col-${item.key}`">{{ item.title }}</Option>
          </Select>
        </Col>
        <Col span="12" class="form-container-item" v-if="asIsaccount">
          <label>社保账号：</label>
          <Input :readonly="isView" type="text" v-model="asAccount" />
        </Col>
        <Col span="12" class="form-container-item" v-if="asIsaccount">
          <label>购买时间：</label>
          <DatePicker :readonly="isView" v-model="asTime" type="datetime" placeholder="请选择社保购买时间" style="width: 100%"></DatePicker>
        </Col>
      </Row>
      <div slot="footer">
  ...
  export default {
    data () {
      return {
        ...
        asTime: null,
        columns: [
          {title: '序号', align: 'center', key: 'as_id', sortable: true},
          {title: '申请人', align: 'center', key: 'username'},
          {
              title: '是否有社保卡', align: 'center', key: 'as_isAccount',
              render: (h, params) => {
                  if (params.row.as_isAccount) {
                    return h('div', '是')
                  } else {
                    return h('div', '否')
                  }
          ...
          {title: '社保卡号', align: 'center', key: 'as_account'},
          {
            title: '操作',
            key: 'handle',
            align: 'center',
              ...
              for (var i = 0; i < this.handleBtns.length; i++) {
                ...
                if (this.handleBtns[i].p_id === 175) {
                  btns.push(h('Button', {
                    ...
                    on: {
                      click: () => {
                        this.showOneEntity(params.row.as_id)
                        this.showModal = true
                        this.modalTitle = '申请修改'
                        this.isView = false
                        this.isEdit = true
                        this.editId = params.row.as_id
                      }
                    }
                  }, '修改'))
                } else if (this.handleBtns[i].p_id === 176) {
                  btns.push(h('Button', {
                    props: {
                      type: 'info',
                      size: 'small'
                ...
                } else if (this.handleBtns[i].p_id === 177) {
                  btns.push(h('Button', {
                    props: {
                      type: 'error',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.deleteEntity(params.row.as_id)
                      }
                    }
                  }, '删除'))
                } else if (this.handleBtns[i].p_id === 178) {
                  btns.push(h('Button', {
                    props: {
                      type: 'warning',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.restoreEntity(params.row.as_id)
                      }
                    }
                  }, '恢复'))
                } else if (this.handleBtns[i].p_id === 179) {
                  btns.push(h('Button', {
                    props: {
                      type: 'success',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.submitEvent(params.row.as_id)
                      }
                    }
                  }, '提交'))
                }
    ...
    methods: {
      asIsaccountChange: function (val) {
        console.log(val)
      },
      handleAdd () {
        this.showModal = true
      ...
      addEntity: function () {
        ...
        axios.request({
          ...
          data: {
            appUser: getUserId(),
            asIsaccount: _this.asIsaccount,
            asAccount: _this.asAccount,
            asTime: dateToStr(new Date(_this.asTime), 'yyyy-MM-dd hh:MM:ss')
          },
          method: 'post'
        }).then(res => {
      ...
      showOneEntity: function (entityId) {
        ...
        axios.request({
          ...
          _this.asIsaccount = res.data.content.as_isAccount,
          _this.asAccount = res.data.content.as_account,
          _this.asTime = res.data.content.as_time
        })
      },
      ...
      getPermissionBtns: function () {
        ...
        for (var item of common.permission) {
          if (item.p_fid === this.$route.meta.id) {
            ...
            if (item.p_id === 175 || item.p_id === 176 || item.p_id === 177 || item.p_id === 178 || item.p_id === 179) {
