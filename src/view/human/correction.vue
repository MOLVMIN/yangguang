      ...
      <TabPane label="我的申请列表" name="name1">
        <Card>
          <div class="search-con">
            ...
            <Button v-for="item in permissionBtns" v-if="item.p_id === 118" @click="handleAdd" class="add-btn" type="primary">{{ item.p_name }}</Button>
          </div>
          <Table
            ref="tablesAsk"
            :data="tableData"
    ...
    <Modal width="800" v-model="showModal" :mask-closable="false">
      ...
      <Row class="form-container">
        <Col span="12" class="form-container-item">
          <label>转正时间：</label>
          <DatePicker v-model="acTime" type="date" format="yyyy-MM-dd" placeholder="请选择转正时间" style="width: 100%"></DatePicker>
        </Col>
      </Row>
      <Row class="form-container">
        <Col span="24" class="form-container-item">
          <label style="width: 125px">工作总结：</label>
          <Input v-model="acWorksubmit" type="textarea" placeholder="请填写工作总结" />
        </Col>
      </Row>
      <div slot="footer">
  ...
  export default {
    data () {
      return {
        ...
        acWorksubmit: '',
        columns: [
          {title: '序号', align: 'center', key: 'ac_id', sortable: true},
          {title: '申请人', align: 'center', key: 'username'},
          {title: '转正时间', align: 'center', key: 'ac_time'},
          {title: '工作总结', align: 'center', key: 'ac_worksubmit'},
          {
            title: '操作',
            key: 'handle',
            align: 'center',
              ...
              for (var i = 0; i < this.handleBtns.length; i++) {
                ...
                if (this.handleBtns[i].p_id === 119) {
                  btns.push(h('Button', {
                    ...
                    on: {
                      click: () => {
                        this.showOneEntity(params.row.ac_id)
                        this.showModal = true
                        this.modalTitle = '申请修改'
                        this.isView = false
                        this.isEdit = true
                        this.editId = params.row.ac_id
                      }
                    }
                  }, '修改'))
                } else if (this.handleBtns[i].p_id === 120) {
                  btns.push(h('Button', {
                    props: {
                      type: 'info',
                      size: 'small'
                ...
                } else if (this.handleBtns[i].p_id === 121) {
                  btns.push(h('Button', {
                    props: {
                      type: 'error',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.deleteEntity(params.row.ac_id)
                      }
                    }
                  }, '删除'))
                } else if (this.handleBtns[i].p_id === 122) {
                  btns.push(h('Button', {
                    props: {
                      type: 'warning',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.restoreEntity(params.row.ac_id)
                      }
                    }
                  }, '恢复'))
                } else if (this.handleBtns[i].p_id === 123) {
                  btns.push(h('Button', {
                    props: {
                      type: 'success',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.submitEvent(params.row.ac_id)
                      }
                    }
                  }, '提交'))
                }
    ...
    methods: {
      ...
      addEntity: function () {
        ...
        axios.request({
          ...
          data: {
            appUser: getUserId(),
            acTime: dateToStr(new Date(_this.acTime)),
            acWorksubmit: _this.acWorksubmit
          },
          method: 'post'
        }).then(res => {
      ...
      showOneEntity: function (entityId) {
        ...
        axios.request({
          ...
          method: 'post'
            ...
            _this.acTime = dateToStr(new Date(res.data.content.ac_time)),
            _this.acWorksubmit = res.data.content.ac_worksubmit
        })
      },
      ...
      getPermissionBtns: function () {
        ...
        for (var item of common.permission) {
          if (item.p_fid === this.$route.meta.id) {
            ...
            if (item.p_id === 119 || item.p_id === 120 || item.p_id === 121 || item.p_id === 122 || item.p_id === 123) {
