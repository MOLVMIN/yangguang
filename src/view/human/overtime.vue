      ...
      <TabPane label="我的申请列表" name="name1">
        <Card>
          <div class="search-con">
            ...
            <Button v-for="item in permissionBtns" v-if="item.p_id === 166" @click="handleAdd" class="add-btn" type="primary">{{ item.p_name }}</Button>
          </div>
          <Table
            ref="tablesAsk"
            :data="tableData"
    ...
    <Modal width="800" v-model="showModal" :mask-closable="false">
      ...
      <Row class="form-container">
        <Col span="12" class="form-container-item">
          <label>起止时间：</label>
          <DatePicker :readonly="isView" v-model="aoTime" type="datetimerange" format="yyyy-MM-dd HH:mm" placeholder="请选择加班起止时间" style="width: 100%"></DatePicker>
        </Col>
        <Col span="12" class="form-container-item">
          <label>加班时长：</label>
          <InputNumber :readonly="isView" :max="24" :min="0.5" :step="0.5" v-model="aoHours" style="width: 100%"></InputNumber>
        </Col>
      </Row>
      <Row>
        <Col span="24" class="form-container-item">
          <label style="width: 125px">加班原因：</label>
          <Input :readonly="isView" v-model="aoResult" type="textarea" placeholder="请填写加班原因" />
        </Col>
      </Row>
      <div slot="footer">
    ...
    data () {
      return {
        ...
        columns: [
          ...
          {title: '加班原因', align: 'center', key: 'ao_result'},
          {title: '开始时间', align: 'center', key: 'ao_starttime'},
          {title: '结束时间', align: 'center', key: 'ao_endtime'},
          {
            title: '操作',
            key: 'handle',
            align: 'center',
            ...
            render: (h, params) => {
              ...
              for (var i = 0; i < this.handleBtns.length; i++) {
                ...
                } else if (this.handleBtns[i].p_id === 168) {
                  btns.push(h('Button', {
                    props: {
                      type: 'info',
                      size: 'small'
                ...
                } else if (this.handleBtns[i].p_id === 169) {
                  btns.push(h('Button', {
                    props: {
                      type: 'error',
                      size: 'small',
                ...
                } else if (this.handleBtns[i].p_id === 170) {
                  btns.push(h('Button', {
                    props: {
                      type: 'warning',
                      size: 'small',
                ...
                } else if (this.handleBtns[i].p_id === 171) {
                  btns.push(h('Button', {
                    props: {
                      type: 'success',
                      size: 'small',
    ...
    methods: {
      ...
      addEntity: function () {
        ...
        axios.request({
          ...
          data: {
            ...
            aoHours: _this.aoHours,
            aoStarttime: dateToStr(new Date(_this.aoTime[0]), 'yyyy-MM-dd hh:MM:ss'),
            aoEndtime: dateToStr(new Date(_this.aoTime[1]), 'yyyy-MM-dd hh:MM:ss')
          },
          method: 'post'
        }).then(res => {
      ...
      showOneEntity: function (entityId) {
        ...
        axios.request({
          ...
          _this.aoHours = res.data.content.ao_hours,
          _this.aoTime = [res.data.content.ao_starttime, res.data.content.ao_endtime]
        })
      },
      ...
      getPermissionBtns: function () {
        ...
        for (var item of common.permission) {
          if (item.p_fid === this.$route.meta.id) {
            ...
            if (item.p_id === 167 || item.p_id === 168 || item.p_id === 169 || item.p_id === 170 || item.p_id === 170) {
