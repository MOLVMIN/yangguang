      ...
      <TabPane label="我的申请列表" name="name1">
        <Card>
          <div class="search-con">
            ...
            <Button v-for="item in permissionBtns" v-if="item.p_id === 142" @click="handleAdd" class="add-btn" type="primary">{{ item.p_name }}</Button>
          </div>
          <Table
            ref="tablesAsk"
            :data="tableData"
...
<script>
  ...
  export default {
    data () {
      return {
        columns: [
          {title: '序号', align: 'center', key: 'gi_id', sortable: true},
              ...
              for (var i = 0; i < this.handleBtns.length; i++) {
                ...
                if (this.handleBtns[i].p_id === 143) {
                  btns.push(h('Button', {
                    ...
                    on: {
                      click: () => {
                        this.showOneEntity(params.row.gi_id)
                        this.showModal = true
                        this.modalTitle = '申请修改'
                        this.isView = false
                        this.isEdit = true
                        this.editId = params.row.gi_id
                      }
                    }
                  }, '修改'))
                } else if (this.handleBtns[i].p_id === 144) {
                  btns.push(h('Button', {
                    props: {
                      type: 'info',
                      size: 'small'
                ...
                } else if (this.handleBtns[i].p_id === 145) {
                  btns.push(h('Button', {
                    props: {
                      type: 'error',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.deleteEntity(params.row.gi_id)
                      }
                    }
                  }, '删除'))
                } else if (this.handleBtns[i].p_id === 146) {
                  btns.push(h('Button', {
                    props: {
                      type: 'warning',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.restoreEntity(params.row.gi_id)
                      }
                    }
                  }, '恢复'))
                } else if (this.handleBtns[i].p_id === 147) {
                  btns.push(h('Button', {
                    props: {
                      type: 'success',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.submitEvent(params.row.gi_id)
                      }
                    }
                  }, '提交'))
                }
      ...
      getPermissionBtns: function () {
        ...
        for (var item of common.permission) {
          if (item.p_fid === this.$route.meta.id) {
            ...
            if (item.p_id === 143 || item.p_id === 144 || item.p_id === 145 || item.p_id === 146 || item.p_id === 147) {
