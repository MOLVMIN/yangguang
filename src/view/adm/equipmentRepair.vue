      ...
      <TabPane label="我的申请列表" name="name1">
        <Card>
          <div class="search-con">
            ...
            <Button v-for="item in permissionBtns" v-if="item.p_id === 54" @click="handleAdd" class="add-btn" type="primary">{{ item.p_name }}</Button>
          </div>
          <Table
            ref="tablesAsk"
            :data="tableData"
  ...
  export default {
    data () {
      return {
        ...
        lbReason: '',
        columns: [
          {title: '序号', align: 'center', key: 'er_id', sortable: true},
          {title: '维修描述', align: 'center', key: 'er_desc'},
          {
            title: '是否送修', align: 'center', key: 'er_is_send',
            render: (h, params) => {
              if (params.row.er_is_send)
                return h('div', '是')
              else
                return h('div', '否')
            }
              ...
              for (var i = 0; i < this.handleBtns.length; i++) {
                ...
                if (this.handleBtns[i].p_id === 55) {
                  btns.push(h('Button', {
                    ...
                    on: {
                      click: () => {
                        this.showOneEntity(params.row.er_id)
                        this.showModal = true
                        this.modalTitle = '申请修改'
                        this.isView = false
                        this.isEdit = true
                        this.editId = params.row.er_id
                      }
                    }
                  }, '修改'))
                } else if (this.handleBtns[i].p_id === 56) {
                  btns.push(h('Button', {
                    props: {
                      type: 'info',
                      size: 'small'
                ...
                } else if (this.handleBtns[i].p_id === 57) {
                  btns.push(h('Button', {
                    props: {
                      type: 'error',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.deleteEntity(params.row.er_id)
                      }
                    }
                  }, '删除'))
                } else if (this.handleBtns[i].p_id === 58) {
                  btns.push(h('Button', {
                    props: {
                      type: 'warning',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.restoreEntity(params.row.er_id)
                      }
                    }
                  }, '恢复'))
                } else if (this.handleBtns[i].p_id === 59) {
                  btns.push(h('Button', {
                    props: {
                      type: 'success',
                      size: 'small',
                    ...
                    on: {
                      click: () => {
                        this.submitEvent(params.row.er_id)
                      }
                    }
                  }, '提交'))
                }
      ...
      getPermissionBtns: function () {
        ...
        for (var item of common.permission) {
          if (item.p_fid === this.$route.meta.id) {
            ...
            if (item.p_id === 55 || item.p_id === 56 || item.p_id === 57 || item.p_id === 58 || item.p_id === 59) {
